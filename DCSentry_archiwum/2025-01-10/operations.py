...
def get_price_trend(product_id):
    ...
    def clean_price(price_str):
        ...
        return float(price_str.replace(' zł', '').replace(',', '.'))
    
    # Clean the prices
    latest_price = clean_price(prices[0][0])
    previous_price = clean_price(prices[1][0])
    ...
    if price_change > 0:
        return f"Price raised by {price_change:.2f}"
    elif price_change < 0:
        return f"Price dropped by {abs(price_change):.2f}"
    else:
        return "Price steady"
...
def add_product(manual=True):
    ...
    if manual:
        ...
        product_name, product_id, description, product_webpage = fetch_product_info(url)
